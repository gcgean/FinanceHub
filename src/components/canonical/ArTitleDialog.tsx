...
import type { ArTitle, Customer, TitleStatus } from "@/api/canonical"
import { Select, SelectTrigger, SelectContent, SelectItem, SelectValue } from "@/components/ui/select"
import { useQuery } from "@tanstack/react-query"
import { listCustomers } from "@/api/canonical"
...
export type ArTitleFormValues = z.infer<typeof Schema>

export function ArTitleDialog(props: {
  open: boolean
  onOpenChange: (open: boolean) => void
  value: ArTitle | null
  loading?: boolean
  onSubmit: (values: ArTitleFormValues) => Promise<void>
}) {
  const defaultValues = useMemo<ArTitleFormValues>(
    () => ({
      customerId: null,
  ...
  const form = useForm<ArTitleFormValues>({ resolver: zodResolver(Schema), defaultValues })
  ...
  const customers = useQuery({ queryKey: ["canonical", "customers", { q: customerSearch }], queryFn: async () => listCustomers({ q: customerSearch }) })
  const customerOptions = useMemo(() => customers.data?.items ?? [], [customers.data])

  const chart = useQuery({ queryKey: ["finance", "chartAccounts", "RECEITA"], queryFn: () => listChartAccounts({ revenueExpense: "RECEITA", planType: "ANALITICA" }) })
    ...
    <Dialog open={props.open} onOpenChange={props.onOpenChange}>
      <DialogContent className="max-w-2xl">
        ...
        >
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-2">
              ...
              <Select value={form.watch("customerId") ?? ""} onValueChange={(v) => form.setValue("customerId", v)}>
                ...
                <SelectContent>
                  {customerOptions.map((s: Customer) => (<SelectItem key={s.id} value={s.id}>{s.name}</SelectItem>))}
                </SelectContent>
              </Select>
            </div>
            <div className="space-y-2">
          ...
          <div className="space-y-2">
            <Label>ID ERP</Label>
            <Input placeholder="ID da integração" {...form.register("erpId")} />
          </div>
