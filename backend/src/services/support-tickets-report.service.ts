...
export function calcularMetricasDetalhadas(
  ...
  const titulares = [...titularMap.entries()].sort((a, b) => b[1] - a[1]).slice(0, TOP_N).map(([nome, count]) => ({ nome, count }));
  ...
  let semNota = 0;
  tickets.forEach(t => {
    if (!temNota(t.nota)) { semNota++; return; }  // NULL ou 0 = sem avaliação
...
export function buildResumoDashboard(m: AiMetricas) {
  return {
    ...
    clientes_promotores_por_qtd_notas_altas: m.clientes_promotores,
    tecnicos_mais_elogiados_por_qtd_notas_altas: m.tecnicos_promotores,
...
- ATENÇÃO À ESCALA: 7 é a nota PADRÃO do sistema (neutra, não é elogio). Notas 0 e ausentes significam SEM AVALIAÇÃO e já foram excluídas — nunca as trate como nota zero/ruim. Só ≤6 é insatisfação real e só ≥8 é elogio real.
- "clientes_insatisfeitos_por_qtd_notas_baixas" e "tecnicos_pior_avaliados_por_qtd_notas_baixas": ordenados por QUANTIDADE de notas ≤6. Em cada item, "qtd" é quantas notas caíram na faixa, "qtd_destaque" quantas foram críticas (≤4), "avaliados" o total avaliado e "pct" o percentual. Pondere: 10 notas baixas em 12 avaliações é grave; 10 em 400 não é.
...
- "amostra_atendimentos_nota_baixa": os piores atendimentos do período. ATENÇÃO: no sistema de origem a observação quase nunca é preenchida (a maioria é lançamento automático, que já foi descartado), então "obs" costuma vir vazio. Quando vier vazio, NÃO invente a causa nem conclua que não houve problema: aponte o cliente, o técnico e o procedimento envolvidos e recomende auditar aquele atendimento no sistema de origem. Quando houver texto, use-o para explicar a nota, citando o caso.
- CARTEIRAS x CLIENTES: nomes como "CONTADORES", "CLIENTE BALCAO", "CERTIFICADO" e "COMMAND SYSTEMS" são agrupamentos/carteiras internas, não empresas únicas. Não os trate como um cliente insatisfeito individual nem sugira ação comercial direcionada a eles.
