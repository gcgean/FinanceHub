...
export async function cashiersRoutes(app: FastifyInstance) {
  app.get(
    "/",
    ...
    async (request) => {
      ...
      if (items.length === 1 && !("items" in (body as object))) {
        ...
        const existing = externalId
          ? await prisma.cashier.findFirst({ where: { companyId, externalId } })
  ...
  app.delete(
    ...
    async (request) => {
      ...
      const count = await prisma.sale.count({ where: { cashierId: params.id } });
      if (count > 0) {
        throw Object.assign(new Error("CANNOT_DELETE_IN_USE"), { statusCode: 400 });
      }

      await prisma.cashier.delete({ where: { id: params.id } });
      return { ok: true };
    }
  );
}
